...
create policy "Org admins create projects" on projects
  for insert with check (
    is_admin()
    or exists(select 1 from memberships where user_id = auth.uid() and role = 'admin_org')
  );
...
create policy "Creator bootstrap project orgs" on project_organizations
  for insert with check (
    exists(
      ...
      where p.id = project_organizations.project_id and p.created_by = auth.uid()
    )
  );
...
create policy "Creator bootstrap project members" on project_members
  for insert with check (
    user_id = auth.uid() and role = 'chef_projet'
    and exists(
      select 1 from projects p
