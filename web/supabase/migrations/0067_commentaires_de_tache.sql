-- ============================================================
-- MIGRATION 0067 — Les commentaires de tâche
-- ============================================================
-- Demande utilisateur du 28/08 : « dans les tâches, il faut pouvoir
-- ajouter des commentaires… et informer à chaque commentaire LEY et la
-- directrice du programme ».
--
-- Ce qui existait et ne suffisait pas : `tasks.comment` (0001), UNE
-- ligne de texte, écrasée à chaque modification et réservée à ceux qui
-- peuvent modifier la tâche. Ce n'est pas une conversation — c'est une
-- note, et la note du dernier qui a cliqué efface celle du précédent.
-- Ce que le terrain demande, c'est un FIL : plusieurs personnes, dans
-- l'ordre, sans que l'une écrase l'autre.
--
-- QUI PEUT ÉCRIRE — arbitrage. La table n'énumère AUCUN rôle projet :
-- commenter suit l'appartenance au projet, comme voir le budget ou le
-- journal. Deux raisons :
--   · commenter n'écrit aucune donnée de pilotage. Un commentaire ne
--     déplace pas une échéance, ne change pas un montant, ne fait pas
--     avancer un pourcentage — il DIT quelque chose, et le reste du
--     circuit continue de décider ;
--   · l'auditeur en bénéficie, et c'est voulu. Une remarque écrite au
--     bon endroit vaut mieux qu'un courriel hors de l'outil ; elle ne
--     lui donne toujours pas le pouvoir de modifier ce qu'il contrôle,
--     ce qui reste la définition du rôle (0038).
-- Conséquence pour `npm run check:rbac` : aucune capacité nouvelle dans
-- RBAC_MATRIX, aucune entrée dans SQL_RULES — les policies ci-dessous
-- n'énumèrent aucun rôle, elles sont donc hors du champ des contrôles 4
-- à 6, exactement comme « See budget lines » ou « See audit ».
--
-- SUPPRIMER : son auteur, ou un administrateur. Personne d'autre — un
-- chef de projet qui effacerait la remarque gênante d'un contributeur
-- viderait le fil de son intérêt. Rien n'est modifiable : un
-- commentaire se corrige en en écrivant un autre, comme dans la vie.

create table if not exists task_comments (
  id uuid primary key default uuid_generate_v4(),
  task_id uuid not null references tasks(id) on delete cascade,
  -- `on delete set null` et non `cascade` : anonymiser une personne
  -- (0063) ne doit pas faire disparaître ce qu'elle a écrit dans un fil
  -- collectif. Le profil devient une pierre tombale, le propos reste et
  -- s'affiche « Compte supprimé ».
...
-- ------------------------------------------------------------
-- De la tâche à son projet, sans récursion
-- ------------------------------------------------------------
-- Les policies ont besoin du projet, et une tâche ne le porte pas : il
-- vit deux tables plus haut (tasks → phases → projects). Écrire la
-- jointure DANS la policy la soumettrait à la RLS de `tasks` et de
-- `phases` — pas une récursion (check-policies n'y verrait rien), mais
-- une dépendance inutile : le droit de commenter dépendrait du droit de
-- lire la ligne de tâche. `security definer`, comme toute lecture
-- déportée depuis la 0022, et `search_path` figé.
create or replace function public.project_of_task(p_task_id uuid)
returns uuid
language sql
stable
security definer
...
revoke all on function public.project_of_task(uuid) from public, anon;
grant execute on function public.project_of_task(uuid) to authenticated;

alter table task_comments enable row level security;

-- Lire : les mêmes yeux que la tâche elle-même.
drop policy if exists "Read task comments" on task_comments;
create policy "Read task comments" on task_comments
  for select using (
    is_admin() or is_project_member(project_of_task(task_id))
  );

-- Écrire : en son nom propre, et seulement sur un projet dont on est
-- membre. `author_id = auth.uid()` n'est pas une formalité — sans lui,
-- un appel direct à PostgREST signerait un commentaire du nom d'un
-- autre.
drop policy if exists "Write task comments" on task_comments;
create policy "Write task comments" on task_comments
  for insert with check (
    author_id = auth.uid()
    and (is_admin() or is_project_member(project_of_task(task_id)))
  );

drop policy if exists "Delete own task comment" on task_comments;
create policy "Delete own task comment" on task_comments
  for delete using (
    author_id = auth.uid() or is_admin()
  );

-- ------------------------------------------------------------
-- L'export RGPD reprend les commentaires écrits par la personne
-- ------------------------------------------------------------
-- La 0064 exporte déjà les commentaires de roadmap ; une table de
-- texte libre signé qui n'y figurerait pas rendrait l'export FAUX au
-- sens de l'article 15 — incomplet, donc trompeur pour celui qui rédige
-- la réponse. La fonction est reprise ici in extenso (c'est ainsi qu'on
-- remplace une fonction) avec un seul bloc de plus.
...
-- `create or replace` conserve les privilèges de la fonction existante ;
-- on les repose quand même, pour que ce fichier dise en entier ce qu'il
-- laisse derrière lui.
