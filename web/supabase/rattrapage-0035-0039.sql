-- ============================================================
-- SOLID'PILOT — Rattrapage 0035 → 0039, à coller en une fois
-- ============================================================
...
--   0039  le droit de regard n'écrit rien : générer un rapport IA exige
--         un rôle projet
--
...
-- 3 bis. Le modèle à deux couches : plus aucune policy d'ÉCRITURE ne
--    doit reposer sur la seule appartenance à une organisation.
--    Attendu : UNE seule ligne, `audit_log / Insert audit` — la trace
--    est écrite par celui qui agit, sous son propre identifiant.
select tablename, policyname, cmd
  from pg_policies
 where schemaname = 'public'
   and cmd <> 'SELECT'
