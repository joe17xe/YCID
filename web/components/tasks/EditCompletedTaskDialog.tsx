  ...
  return (
    <>
      <button
        ...
        aria-label={`Modifier la tâche terminée ${task.title} (admin YCID / LEY)`}
        title="Modifier cette tâche terminée (admin YCID / LEY)"
      >
        <Pencil size={12} aria-hidden="true" /> Modifier
      </button>

      <Modal open={step > 0} onClose={close} busy={pending} maxWidth="max-w-lg"
        title={step === 3 ? "Modifier la tâche" : "Tâche terminée"}>
        <>
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <div>
                  <label htmlFor={`${uid}-motif`} className="block text-sm font-medium mb-1" style={{ color: "#17211D" }}>Motif de la modification *</label>
                  <textarea id={`${uid}-motif`} value={motif} onChange={e => setMotif(e.target.value)} rows={2} required
            ...
            {step === 3 && (
              <form className="space-y-4" onSubmit={e => { e.preventDefault(); submit() }}>
                ...
                <div>
                  <label htmlFor={`${uid}-comment`} className="block text-sm font-medium mb-1" style={{ color: "#17211D" }}>Commentaire</label>
                  <textarea id={`${uid}-comment`} value={form.comment} onChange={e => setForm({ ...form, comment: e.target.value })} rows={2}
                    className={inputCls} style={{ borderColor: "#E3E6E2" }} />
                </div>
