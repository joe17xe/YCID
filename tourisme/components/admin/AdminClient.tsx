...
export default function AdminClient({
  ...
  const charger = useCallback(async () => {
    ...
    const { data: t } = await sb
      .from("territoires")
      .select(
        "id, slug, marque, contact_tel, contact_whatsapp, contact_email, etat_acces",
      )
  ...
  if (!pret)
    return (
      <main className="grid min-h-screen place-items-center">
        <LoaderCircle
          className="animate-spin text-[var(--encre-3)]"
    ...
    return (
      <main className="mx-auto max-w-md px-4 py-16">
        ...
        <p className="mt-2 text-[14px] text-[var(--encre-2)]">
          Recevez un lien de connexion par e-mail. L&rsquo;accès en écriture est
          réservé aux éditeurs du territoire.
        </p>
        <form
        ...
        {info ? (
          <p className="mt-3 text-[13.5px] text-[var(--encre-2)]">{info}</p>
        ) : null}
      </main>
    );
    ...
    <main className="mx-auto max-w-3xl space-y-8 px-4 py-8">
      ...
      {territoire ? (
        ...
        <p className="text-[14px] text-[var(--encre-2)]">
          Territoire illisible — votre compte n&rsquo;est probablement pas
          encore éditeur (table{" "}
          <code className="mono">territoire_editeurs</code>, voir le guide de
          déploiement).
        </p>
      )}
      {territoire ? (
      ...
      <button
        ...
        onClick={async () => {
          const { error } = await sb
            ...
            .eq("id", territoire.id);
          onSaved(
            error ? `Erreur : ${error.message}` : "Territoire enregistré.",
          );
        }}
      >
...
function LigneParcours({
  ...
  const importerGpx = async (file: File) => {
    try {
      ...
      for (const f of fc.features) {
        ...
        if (g?.type === "LineString") {
          ...
          for (const seg of g.coordinates)
            for (const c of seg) coords.push([c[0], c[1]]);
        }
      }
      if (coords.length < 2) {
      ...
      setGpxInfo(
        "Fichier illisible — exportez un GPX ou un KML converti en GPX.",
      );
    }
  };
      ...
      <div className="flex flex-wrap items-center gap-2.5">
        ...
        <label className="btn btn-surface !min-h-[40px] cursor-pointer !py-2 text-[13px]">
          ...
          <input
            ...
            className="hidden"
            onChange={(e) =>
              e.target.files?.[0] && importerGpx(e.target.files[0])
            }
          />
        </label>
        {gpxInfo ? (
          <span className="text-[12.5px] text-[var(--encre-2)]">{gpxInfo}</span>
        ) : null}
      </div>
    </div>
  );
  ...
  return (
    <section className="space-y-3">
      ...
      <p className="text-[12.5px] text-[var(--encre-2)]">
        Coordonnées au format Google Maps (« 33.5342, 35.5648 ») — l’ordre est
        détecté, et tout point hors du Liban est refusé.
      </p>
      {pois.map((o) => (
  ...
  return (
    <div className="card flex flex-wrap items-end gap-2.5 p-3.5">
      <p className="w-full text-[14px] font-bold sm:w-auto sm:flex-1">
        {tx(o.nom, "fr")}{" "}
        <span className="mono text-[11px] font-normal text-[var(--encre-3)]">
          ({o.type})
        </span>
      </p>
      <label className="text-[12px] font-semibold">
      ...
      {err ? (
        <span className="w-full text-[12.5px] font-semibold text-[var(--danger)]">
          {err}
        </span>
      ) : null}
    </div>
